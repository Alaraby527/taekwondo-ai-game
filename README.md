...
页面共有 **两处** 二维码占位框（引导页 + 游戏结束面板），都需要替换成官方号二维码图片：

1. 把二维码图片放到同目录，例如 `qrcode.png`（建议白底、正方形、边长 500px 以上）。
2. 打开 `index.html`，找到 **两处** 注释：

```html
<!-- 替换为官方二维码图片：<img src="qrcode.png" alt="抖音校园官方号二维码"> -->
```

3. 把注释替换成（**两处都换**）：

```html
<img src="qrcode.png" alt="抖音校园官方号二维码">
```

4. 刷新页面确认引导页和胜利/失败面板都显示二维码。
