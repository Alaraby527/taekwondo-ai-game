...
| `JEV_MODEL` | 空 | 可选，指定模型 |
| `BUDGET_PER_FIGHT` | `120` | **每局「真实调用」**上限（缓存命中不计入），超了返回 `degraded` |
...
1. **决策降频**：前端每 1.4 秒才问一次
   - 速战单局（30s）≈ **21 次请求**
   - 正式赛五局三胜（约 300s）≈ **214 次请求**
2. **局面缓存**：状态量化（距离 40 单位、血量 20%、得分 2 分、时间 8 秒…）后命中同一条。
   **缓存命中零成本，因此不消耗预算、也不受闸门限制**（顺序很重要：先查缓存再扣预算）
3. **每局预算** `BUDGET_PER_FIGHT`：默认 **120 次真实调用**，超了立刻降级。
   只有真正打到 Jev 的请求才计数，所以缓存命中率高时能撑完整场正式赛
4. **每日上限** `MAX_CALLS_PER_DAY`：默认 **3000 次真实调用**，公开部署的硬止损。
   按速战约 10 次真实调用/局估算，约合 **300 局/天**

> 实测：每局预算默认 120、每日 3000，若一场都是速战，用量约 10 次/局 → 每天约 300 局，足够摊位使用；达到上限后前端会退避到 12 秒一次并最终回到纯本地 AI，**不会影响游戏可玩性**。
